...
def data_loader(file):
    ...
    with codecs.open(file2, 'r') as f1, codecs.open(file3, 'r') as f2:
        lines1 = f1.readlines()  # 作为列表返回文件中的所有行
        lines2 = f2.readlines()
        for line in lines1:
    ...
    with codecs.open(file1, 'r') as f:  # 文件读尽量用codecs.open方法，一般不会出现编码的问题
        ...
        for line in content:
            ...
            h_ = entity2id[triple[0]]  # 找到train.txt中三元组的每个元素对应的数字编号
            t_ = entity2id[triple[1]]
            r_ = relation2id[triple[2]]

            triple_list.append([h_, t_, r_])  # 储存三元组的数字编号

            entity_set.add(h_)  # 储存头实体的数字编号
            entity_set.add(t_)  # 储存尾实体的数字编号

            relation_set.add(r_)  # 储存关系的数字编号
...
def distanceL2(h, r, t):
    # 为方便求梯度，去掉sqrt
    return np.sum(np.square(h + r - t))  # 对应空间坐标中的向量
...
class TransE:
    ...
    def train(self, epochs):
        nbatches = 400  # 先验条件
        batch_size = len(self.triple_list) // nbatches  # 计算每个batch有多少样本
        ...
        for epoch in range(epochs):
            ...
            for k in range(nbatches):
                Sbatch = random.sample(self.triple_list, batch_size)  # 随机抽取 “batch_size” 个正样本三元组（(h, r, t)）
                Tbatch = []  # 存储负样本三元组
    ...
    def update_embeddings(self, Tbatch):
        # 由于每次训练的 Tbatch 只是整个数据的一部分，因此避免不必要的拷贝，只更新 batch 内涉及的实体和关系。
        entity_updated = {}
        relation_updated = {}
        for triple, corrupted_triple in Tbatch:
            ...
            # 只对当前 Tbatch 中出现的实体和关系进行拷贝，避免重复更新
            ...
            if err > 0:
                ...
                # 对于 L1 范数，使用符号函数 (sign function) 将梯度调整为 1 或 -1，而不是直接使用梯度值
                if self.L1:
